...
- Current Phase 2 includes the bounded AI PR Evidence Gate workflows, the exact `PRODUCT-RELEASE-001` release-candidate slice, and the `PRODUCT-MARKET-001` public prototype. The market prototype loads separately supplied v2 runtime authority, uses the exact base configuration, binds an exact pull-request head, runs only configured bounded commands, records canonical Verification Receipts, and passes normalized inputs to deterministic kernel evaluation.
- The market prototype authorizes only its declared checkout, command, environment, output, and artifact boundaries. It does not authorize arbitrary repository inspection, arbitrary commands, GitHub writes, adapters, API, MCP, hosted service behavior, model providers, Inference Zone behavior, LLM judgment, or probabilistic authority.
- The mechanically guarded production surface is exactly seven packages: `contracts`, `kernel`, `trusted-config`, `release-orchestrator`, `evidence-gate`, `static-evaluator`, and `verification-runner`. New packages or edges still require an explicit Machine Task Contract.
...
The current Phase 2 product direction includes bounded local delivery for static inputs, sanitized read-only GitHub metadata, the exact externally configured release-candidate path, and the separately authorized market-prototype path. The market path may checkout and execute the exact target only through its bounded runner after v2 runtime authority and exact base/head binding succeed. It must keep Claims, GitHub-reported Observations, Proofrail-executed Verification Receipts, Evidence, scope, and review needs separate.

Current work MUST NOT expand the implemented bounded paths into claims of general repository inspection, complete sandboxing, unrestricted verification execution, adapters, hosted delivery, Inference Zone implementation, product readiness, trusted release status, external release acceptance, or applicability beyond the supplied v2 Trusted Configuration, Policy, and Evidence Contract. An authoritative Proofrail Verdict is limited to that exact evaluation; repository engineering review PASS is never a product Verdict.
