...
The offline fixture commits a sanitized frozen snapshot, canonical kernel input, and canonical Evidence Bundle. `PRODUCT-RELEASE-001` deliberately left base-SHA Evidence missing because its contract prohibited query expansion. `PRODUCT-RELEASE-002` separately authorizes adding only `baseRefOid` to the existing metadata query. The collector validates it as `baseOid`; orchestration binds it to configured `target.baseSha` and creates the Observation from that GitHub result rather than configuration. The authorized live result and offline golden are byte-identical and the unchanged kernel returns `ADMISSIBLE`. Live collection remains environmental and is not generally claimed byte-reproducible.
...
Any implementation beyond the v0.1 static-input CLI, bounded v0.2 local importer, and exact `PRODUCT-RELEASE-001` plus `PRODUCT-RELEASE-002` configured slice requires a later valid Machine Task Contract that defines exact write scope, forbidden scope, acceptance criteria, required verification, and stop conditions.
