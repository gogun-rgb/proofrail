...
The bounded current-package guard freezes the exact seven-package dependency graph: `kernel -> contracts`, `release-orchestrator -> kernel + trusted-config`, `evidence-gate -> release-orchestrator + trusted-config + verification-runner`, and `static-evaluator -> kernel`; `contracts`, `trusted-config`, and `verification-runner` have no workspace imports. The only allowed external production dependency is the exact YAML parser used by `trusted-config`. It freezes each package's production Node import allowlist and exact manifest loading fields (type, exports, bin, and the required absence of alternate main, module, browser, or package-import-map entry points).
...
The guard rejects direct eval, new Function, escaped or aliased require, computed-property require invocation, direct createRequire references, non-exact child-process imports, and other directly named subprocess loader calls. It separately permits only the exact `verification-runner` subprocess imports and argument-array `spawn` boundary used for authorized verification. These are repository engineering architecture findings; they do not create product execution authority.
