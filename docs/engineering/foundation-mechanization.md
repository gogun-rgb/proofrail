...
The enforced surface is intentionally frozen to six current package directories and their exact manifest names. `contracts` has no workspace edge; `kernel` may depend only on `contracts`; `trusted-config` has no workspace edge; `release-orchestrator` may depend only on `kernel` and `trusted-config`; `evidence-gate` may depend only on `release-orchestrator`; and `static-evaluator` may depend only on `kernel`. Their Node imports are exact allowlists and no external bare runtime import is approved. Runtime manifest checking covers `dependencies`, `optionalDependencies`, `peerDependencies`, `bundleDependencies`, and `bundledDependencies`, while `devDependencies` are outside the production boundary. Relative containment recognizes slash, backslash, and mixed separators on every host; absolute and URL targets use fixed diagnostic categories instead of exposing host paths.
