...
The verifier checks required files and artifacts, local documentation links and anchors, exact-set canonical terminology and Verdict projections, governance configuration, Machine Task Contract instances, the separate HARN_ harness and product-owned reason-code registries, product emitted-code and reference drift, exact LF checkout bytes, generated projection freshness, authority-index routes, JSON-output parseability, two strict Clean Agent records, governance tests, the bounded current-package architecture guard and its synthetic tests, the deterministic product fixture corpus and generated inventory, package tests, Phase 1 type checking, diff whitespace, and repository identity hygiene. It is not a product runtime and does not provide independent acceptance.
...
pnpm product:fixture-inventory
